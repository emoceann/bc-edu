...
async def newbie_articles(callback: types.CallbackQuery, state: FSMContext):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1).add(
        *(i for i in text['buttons4'].split('\n')))
...
async def newbie_knowledge_choose(msg: types.Message, state: FSMContext):
    text = get_template('newbie_knowledge_base.html', content_list=dict(buttons2={}))
